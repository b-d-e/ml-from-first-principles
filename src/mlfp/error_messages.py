...
def matrix_shape_mismatch(shape_a: tuple[int, int], shape_b: tuple[int, int]) -> str:
...
def matrix_incompatible_shapes(
    self_shape: tuple[int, int], other_shape: tuple[int, int]
) -> str:
    err = f"Matrices must have compatible shapes - \n Matrix A is {self_shape},\
    Matrix B is {other_shape}\n{self_shape[1]} != {other_shape[0]}"
    if self_shape[0] == other_shape[1]:
        err += "\nHint: It looks like you might want to product these the other\
                way around?"
    return err
